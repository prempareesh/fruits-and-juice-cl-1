-- ==========================================
-- JUICY APP: PRODUCTION STABILITY VALIDATION (V5)
-- Concurrency Safe RPC with Row-Level Locking
-- ==========================================
...
BEGIN
  -- 1. VALIDATION & LOCKING: Check stock with ROW LOCKING
  -- We lock all requested products upfront to prevent race conditions
  FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(
    ...
    FOR UPDATE; -- CRITICAL: Lock row for this transaction
  ...
  INSERT INTO public.orders (
    user_id, address, total_amount, payment_type, status, 
    payment_status, delivery_lat, delivery_lng, delivery_distance_km, 
    delivery_fee, customer_name, customer_phone, tracking_steps
  ) VALUES (
    ...
    p_latitude, p_longitude, p_distance_km, p_delivery_fee, 
    p_customer_name, p_customer_phone,
    jsonb_build_array(jsonb_build_object(
      'status', p_initial_status, 'timestamp', now(), 
      'description', 'We have received your order.', 'label', 'Order Placed'
    ))
  ...
  FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(
    ...
    price_at_time NUMERIC, subtotal NUMERIC
  ) LOOP
