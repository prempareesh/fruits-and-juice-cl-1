...
export default function CategoryDetailScreen() {
  ...
  useEffect(() => {
    async function fetchCategoryProducts() {
      ...
      try {
        ...
        if (lowerId === 'trending') {
          query = query.or('is_trending.eq.true,is_featured.eq.true');
        } else if (lowerId === 'others') {
          // Others: anything not fruit, veggie, or juice
          query = query.not('category', 'ilike', '%fruit%')
                      .not('category', 'ilike', '%vegetable%')
                      .not('category', 'ilike', '%juice%');
        } else {
          // Robust matching for common categories
          const searchPattern = lowerId.endsWith('s') ? lowerId.slice(0, -1) : lowerId;
          query = query.ilike('category', `%${searchPattern}%`);
        }
  ...
  const categoryName = useMemo(() => {
    ...
    if (id.toLowerCase() === 'trending') return 'Trending Now';
        ...
        onRefresh={() => {
          ...
          if (currentId) {
             ...
             supabase.from('products').select('*')
               .ilike('category', `%${currentId.toLowerCase().endsWith('s') ? currentId.slice(0, -1) : currentId}%`)
               .then(({ data }) => {
                 setCategoryProducts(data || []);
                 setLoading(false);
               });
